...
## Compare AHC / New Method vs ALR reference choices, CLR, ILR
##
## Main fixes retained from previous version:
...
##
## New addition:
## ALR is now evaluated under 5 reference choices:
##   1. ALR_nondiff_ref : original/current setting,
##                        choose true non-differential component
##   2. ALR_random_ref  : randomly choose any component
##   3. ALR_signal_ref  : choose true differential component
##   4. ALR_last_ref    : choose last component
##   5. ALR_minvar_ref  : choose component with smallest variance
##                        across samples after zero replacement
...
## ============================================================
## 6. Transformations: AHC, ALR, CLR, ILR
## ============================================================
...
## ============================================================
## 7. ALR reference-selection functions
## ============================================================
...
choose_random_reference <- function(true_effects) {
  
  p <- length(true_effects)
  
  ## Randomly choose one component, regardless of whether it is truly significant.
  ref_component <- sample(seq_len(p), size = 1)
  
  return(ref_component)
}


choose_signal_reference <- function(true_effects) {
  
  signal_index <- which(true_effects)
  
  if (length(signal_index) == 0) {
    stop("No truly differential feature is available as ALR reference.")
  }
  
  ## Choose the last known significant component.
  ## This is deterministic and does not add extra randomness.
  ref_component <- tail(signal_index, 1)
  
  return(ref_component)
}


choose_last_reference <- function(true_effects) {
  
  ## Choose the last component.
  ref_component <- length(true_effects)
...
choose_minvar_reference <- function(data,
  ...
  ## Choose the component with the smallest variance across samples
  ## after the same zero replacement and row normalization used by ALR.
  ...
  ref_component <- which.min(var_vec)
...
get_alr_reference_table <- function(data,
  ...
  ref_nondiff <- choose_nondiff_reference(true_effects)
  ref_random <- choose_random_reference(true_effects)
  ref_signal <- choose_signal_reference(true_effects)
  ref_last <- choose_last_reference(true_effects)
  ref_minvar <- choose_minvar_reference(
    data = data,
    group_factor = 1,
    zero_constant = zero_constant
  )
  ...
  ref_tbl <- tibble(
    ...
    mutate(
      alr_ref_is_signal = true_effects[alr_ref],
      alr_ref_variance = as.numeric(var_vec[alr_ref])
    )
...
## ============================================================
## 8. Testing and metric functions
## ============================================================
...
run_one_dataset <- function(data,
  ...
  ## Original/current ALR reference used in the previous code.
  ## It is still used for ILR dimension-matching exactly as before.
  alr_ref_original <- choose_nondiff_reference(true_effects)
  
  ## New ALR reference table with all 5 requested reference choices.
  alr_ref_tbl <- get_alr_reference_table(
    data = data,
    true_effects = true_effects,
    zero_constant = 0.5
  )
  ...
  ## --------------------
  ## ALR under multiple reference choices
  ## --------------------
  
  for (i in seq_len(nrow(alr_ref_tbl))) {
    
    this_transformation <- alr_ref_tbl$transformation[i]
    this_strategy <- alr_ref_tbl$alr_ref_strategy[i]
    this_ref <- alr_ref_tbl$alr_ref[i]
    this_ref_is_signal <- alr_ref_tbl$alr_ref_is_signal[i]
    this_ref_variance <- alr_ref_tbl$alr_ref_variance[i]
    
    transformed_ALR <- alr_transformation(
      data = data,
      ref_component = this_ref
    )
    
    p_ALR <- run_feature_ttests(transformed_ALR)
    
    ## The ALR transformed data has p - 1 coordinates because the
    ## reference component is removed. To preserve the same feature-level
    ## comparison logic as the original code, remove the reference component
    ## from the truth vector as well.
    ...
    method_results <- bind_rows(
      ...
      make_method_result(
        ...
        alr_ref = this_ref,
        alr_ref_strategy = this_strategy,
        alr_ref_is_signal = this_ref_is_signal,
        alr_ref_variance = this_ref_variance,
        significance_level = significance_level
      )
    )
  }
  ...
  ## This keeps the same p-1 dimensional comparison structure as the
  ## original code, using the original non-differential ALR reference.
  truth_ILR <- true_effects[-alr_ref_original]
  ...
  method_results <- bind_rows(
    ...
    make_method_result(
      ...
      alr_ref_is_signal = true_effects[alr_ref_original],
      alr_ref_variance = NA_real_,
      significance_level = significance_level
    )
  )
...
for (param_set in seq_len(total_param_sets)) {
  ...
  for (rep_id in seq_len(n_simulation)) {
    ...
    ## This preserves the simulated dataset generation.
    ## The random ALR reference is also reproducible within each replicate.
  ...
  summary_this_setting <- replicate_results_this_setting %>%
    ...
    summarize(
      ...
      mean_alr_ref = safe_mean(alr_ref),
      prop_alr_ref_signal = safe_mean(as.numeric(alr_ref_is_signal)),
      mean_alr_ref_variance = safe_mean(alr_ref_variance),
      .groups = "drop"
    )
...
replicate_file <- file.path(
  outdir,
  "replicate_results_AHC_ALR_reference_choices_CLR_ILR_MSeqU_100rep_corrected.csv"
)

summary_file <- file.path(
  outdir,
  "summary_results_AHC_ALR_reference_choices_CLR_ILR_MSeqU_100rep_corrected.csv"
)
...
## ============================================================
## 12. Quick sanity checks printed at the end
## ============================================================
...
cat("\nQuick check: ALR reference behavior\n")

alr_reference_check <- all_replicate_results %>%
  filter(grepl("^ALR_", transformation)) %>%
  group_by(transformation, alr_ref_strategy) %>%
  summarize(
    n_rows = n(),
    mean_alr_ref = mean(alr_ref, na.rm = TRUE),
    min_alr_ref = min(alr_ref, na.rm = TRUE),
    max_alr_ref = max(alr_ref, na.rm = TRUE),
    prop_ref_signal = mean(as.numeric(alr_ref_is_signal), na.rm = TRUE),
    mean_ref_variance = mean(alr_ref_variance, na.rm = TRUE),
    median_ref_variance = median(alr_ref_variance, na.rm = TRUE),
    .groups = "drop"
  )

print(alr_reference_check)
